...
struct menu_data {
public:
    menu_data(int lvl, int in, bool repeat, int (easyvr::*cb)(int),
              const std::forward_list<voice_messages_t> &help): level(lvl),
              intro(in), intro_repeat(repeat), help_infos(help), callback(cb) {};
...
private:
    ...
    static const std::unordered_map<int, menu_data> menus;
    ...
    void reset(void);
    void initialize(void) { reset(); initialize_serial(); initialize_baudrate(); recover_vr(); }
    void initialize_serial(void);
    void initialize_baudrate(void);
    void recover_vr(void);
